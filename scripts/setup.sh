...
# ---- centralized logs (compose profile: logs) ----
# Both files are generated, not tracked. The Grafana datasource is removed when
# logs are off, otherwise Grafana logs an error about a datasource whose plugin
# is not installed.
datasource_file="grafana/provisioning/datasources/victorialogs.yml"
if [ "${LOGS_ENABLED:-false}" = "true" ]; then
  : "${LOGS_SERVER_NAME:?LOGS_ENABLED=true requires LOGS_SERVER_NAME in .env}"
  echo "setup: rendering vector/vector.yaml"
  mkdir -p vector
  render vector/vector.yaml.tmpl vector/vector.yaml

  echo "setup: writing $datasource_file"
  cat > "$datasource_file" <<'DATASOURCE'
# Generated by scripts/setup.sh when LOGS_ENABLED=true. Do not edit by hand.
apiVersion: 1
datasources:
  - name: VictoriaLogs
...
DATASOURCE
  ...
  if [ "${GRAFANA_PLUGINS:-}" != "victoriametrics-logs-datasource" ]; then
    echo "setup: NOTE - set GRAFANA_PLUGINS=victoriametrics-logs-datasource in .env, otherwise Grafana cannot render the VictoriaLogs datasource"
  fi
else
  rm -f vector/vector.yaml "$datasource_file"
fi
