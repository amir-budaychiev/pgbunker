...
# Reads DOMAIN, LE_EMAIL, PGBOUNCER_TLS from .env.
#
...
echo "nginx-setup: DOMAIN=$DOMAIN PGBOUNCER_TLS=$PGBOUNCER_TLS"
...
chmod 644 "\$cert_dst/fullchain.pem" "\$cert_dst/privkey.pem"
...
if [ "$PGBOUNCER_TLS" = "true" ]; then
  echo "  DB:      postgresql://USER:PASS@$DOMAIN:6432/DB?sslmode=require"
else
  echo "  DB:      postgresql://USER:PASS@$DOMAIN:6432/DB   (no TLS; use UFW source-IP whitelist or VPN)"
fi
